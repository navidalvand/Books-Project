...
class BooksController {
  async createBook(req, res, next) {
    try {
      const user = req.user;
      if (!user.author) throw {status : 400 , message : "please switch to author account first"}
  ...
  async updateBook (req , res , next) {
    try {
        ...
        const user = req.user
        const {name , pageCount} = req.body
        if(!name && !pageCount) throw {status : 400 , message : "please update at least one field"}
        const findBook = await BookModel.findOne({bookAuthor : user._id , _id : bookID})
        if(!findBook) throw {status : 404 , message : `book with ${bookID} ID not found`}
        const updatedBook = await BookModel.updateOne({_id : bookID } , {name , pageCount})
        res.status(200).json({
            status : 200,
  ...
  async getAllBooksByAuthor (req , res , next) {
    try {
        const user = req.user
        if(!user.author) throw {status : 400 , message : "please switch to author aacount first"}
